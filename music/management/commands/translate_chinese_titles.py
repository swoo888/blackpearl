...
logger = logging.getLogger('blackpearl.music.translate_chinese_titles')
...
class Command(base.NoArgsCommand):
    ...
    def handle_noargs(self, **options):
        if not options['silentmode']:
            logging.getLogger('blackpearl').setLevel(logging.INFO)
        if options['extrasilent']:
            logging.getLogger('blackpearl').setLevel(logging.ERROR)
        if options['debugmode']:
            logging.getLogger('blackpearl').setLevel(logging.DEBUG)
